...
class AuthManager {
  ...
  async setCurrentUser(user) {
    ...
    // Check if user is admin (by checking a role or admin table)
...
class NexdivApp {
  constructor(config = {}) {
    this.config = {
      supabaseUrl: config.supabaseUrl || 'https://YOUR_PROJECT_ID.supabase.co',
      supabaseKey: config.supabaseKey || 'YOUR_ANON_PUBLIC_KEY',
      ...config
    };
  ...
  async initialize() {
    try {
      ...
      this.sb = createClient(this.config.supabaseUrl, this.config.supabaseKey);
...
async function initializeNexdivApp(config = {}) {
  nexdivApp = new NexdivApp(config);
