// ============================================================
// main.js — Frontend Dynamic Content and Modules Logic
// ============================================================
...
function initTheme() {
  ...
  const currentTheme = localStorage.getItem('nexdiv-theme') || 'dark';
...
async function loadSettings() {
  ...
  // WhatsApp CTA customization
  const waBtn = document.getElementById('whatsapp-floating-button');
  if (waBtn && siteSettings.whatsapp_number) {
...
function initDomainChecker() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    setTimeout(() => {
      ...
      const exts = [
        { name: '.com', priceKey: 'domain_price_com', fallback: '9.99' },
        { name: '.net', priceKey: 'domain_price_net', fallback: '11.99' },
        { name: '.org', priceKey: 'domain_price_org', fallback: '12.99' },
        { name: '.xyz', priceKey: 'domain_price_xyz', fallback: '1.99' },
        { name: '.info', priceKey: 'domain_price_info', fallback: '4.99' }
      ];
      ...
      exts.forEach((ext, index) => {
        ...
        const price = siteSettings[ext.priceKey] || ext.fallback;
        ...
        row.innerHTML = `
          ...
          <div class="domain-status-group">
            ${isAvailable 
              ? `<span class="domain-price">$${price}/yr</span>
                 <button class="btn-order-domain" onclick="triggerDomainPurchase('${cleanDomain}${ext.name}', '${price}')">Register</button>`
...
function triggerDomainPurchase(domainName, price) {
  openOrderProjectModal({
    id: 'domain-' + domainName.replace(/\./g, '-'),
    title: `Domain Registration (${domainName} - $${price}/yr)`
  });
}
